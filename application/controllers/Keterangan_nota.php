...
class Keterangan_nota extends CI_Controller {
		
	public function __construct() {
		parent::__construct();
		
		$this->load->model('keterangan_nota_model');
	} 
	...
	public function index(){		
		...
		$this->jumlahData 		= $this->keterangan_nota_model->getCount("",$like);		
		$config['total_rows'] 	= $this->jumlahData;		
		$config['per_page'] 	= 10;		
		
		$this->pagination->initialize($config);	
		$this->showData = $this->keterangan_nota_model->showData("",$like,$order_by,$config['per_page'],$this->input->get('per_page'));
		$this->pagination->initialize($config);
	...
	public function add_data(){
		...
		if ($this->form_validation->run() == FALSE)	{
			...
			$query = $this->keterangan_nota_model->insert($data);							
	...
	public function edit($IdPrimaryKey){
		...
		$this->oldData = $this->keterangan_nota_model->getData($where);	
	...
	public function edit_data(){
		...
		if ($this->form_validation->run() == FALSE)	{
			...
			$query = $this->keterangan_nota_model->update($where,$data);							
			$status = array('status' => true , 'redirect_link' => base_url()."".$this->uri->segment(1));
		}
	...
	public function delete($IdPrimaryKey){
		$where = array('id_keterangan' => $IdPrimaryKey);
		$delete = $this->keterangan_nota_model->delete($where);		
