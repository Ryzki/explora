...
class Cetak extends CI_Controller {
	...
	public function struk($IdPrimaryKey){
		...
		foreach($this->dataDisticntBarang as $dataDistinct){
			...
			$this->cetak .='
			<center ><b style="font-size:16px;">'.$this->dataProfil->NAMA_TOKO .'</b></center>
			...
			$this->cetak .='<table  width="100%" cellpadding="3" style="font-size:16px;">';
		...
		$this->showLaporan = "";
			...
			foreach($this->showJenisBayar as $dataJenisBayar ){
				...
				foreach($this->showData as $dataKas){
					$this->showLaporan.= "<table width='100%'>";	
					
					$this->showLaporan.= "<tr><td>".$dataKas->JAM_BAYAR_INDO."</td><td>, WO : ".$dataKas->NO_WO."</td><td>, ".$dataKas->NAMA_KARYAWAN."</td></tr>";	
					$this->showLaporan.= "<tr><td colspan='2'>".$dataKas->JENIS_BAYAR."</td><td>, ".$this->format_rupiah($dataKas->JUMLAH_KAS)."</td></tr>";	
				...
				$jumlahKas= 0;
				$this->showLaporan.= " <table width='100%'>       <tbody>";	
				foreach($this->showJenisBayar as $dataJenisBayar ){
					...
					$this->showLaporan.= "<tr><td colspan='2' align=''>Jumlah</td><td ><b>".$this->format_rupiah($jumlahKas)."</b></td></tr>";		
				...
				if($this->showDataCancelBarang){
					$this->showLaporan.="<b>Barang yang Void tanggal ".$this->input->get('date')."</b><br><br>";
				}
				
				$this->showLaporan.= " <table class='table table-bordered'>   <thead>  <tr>     <th >No.</th> <th >Nomor WO</th>    <th >  Jam Cancel Barang</th>   <th >Status Bayar Saat Cancel </th> <th >Harga Barang </th></tr>    </thead>       <tbody>";	
				$ii=1;
				$jumlahUangVoid = 0;
				foreach($this->showDataCancelBarang as $dataCancelBarang ){
						...
						$this->showLaporan.= "<tr><td align=center>".$ii."</td><td>".$dataCancelBarang->no_wo."</td><td>".$dataCancelBarang->JAM_CANCEL_INDO."</td><td>".$dataCancelBarang->STATUS_BAYAR_CANCEL."</td><td align=right> ".$this->format_rupiah($dataCancelBarang->TOTAL_HARGA)."</td></tr>";							
						$ii++;
